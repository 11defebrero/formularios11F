...
get_charlas_concertadas_original <- function(file_id, filename=NULL) {
  ...
  get_drive_sheet(file_id, filename) %>%
    dplyr::transmute(
      ...
      centro = coalesce(`Nombre del centro (check)`, `Nombre del centro educativo`),
      com_autonoma = `Comunidad Autónoma`,
      provincia = coalesce(`Provincia (check)`, `Provincia`),
      localidad = `Localidad`,
      codpostal = `Código Postal`,
      ponente = `Nombre y apellidos`,
      institucion_ponente = `Institución a la que pertenece`,
      email_ponente = `Email de contacto`,
      titulo_charla = `Título de la charla`,
      formato = `Tipo de charla`,
      niveles = `Nivel (o niveles) en el que se impartirá la charla`,
